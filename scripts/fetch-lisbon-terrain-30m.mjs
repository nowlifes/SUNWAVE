...
// One fine grid per region, not one rectangle over everything: a box from
// Caparica to Saldanha would be ~80% river and sea, and several times the
// bundle weight, for cells no shadow is ever computed in.
//   node scripts/fetch-lisbon-terrain-30m.mjs [--region lisbon|caparica|almada]
const SOUTH = ([lat, lng]) => lat < 38.6925 || (lng > -9.185 && lat < 38.7);
const REGIONS = {
  lisbon: { exportName: 'lisbonTerrain30', keep: (p) => !SOUTH(p) },
  caparica: { exportName: 'caparicaTerrain30', keep: (p) => SOUTH(p) && p[1] < -9.2 },
  almada: { exportName: 'almadaTerrain30', keep: (p) => SOUTH(p) && p[1] >= -9.2 },
};
const regionArg = process.argv.indexOf('--region');
const REGION = regionArg > 0 ? process.argv[regionArg + 1] : 'lisbon';
if (!REGIONS[REGION]) throw new Error(`unknown region ${REGION} — one of ${Object.keys(REGIONS).join(', ')}`);
const { exportName: EXPORT_NAME, keep: KEEP } = REGIONS[REGION];
const OUT_FILE = join(__dirname, `../src/data/${EXPORT_NAME}.ts`);
...
function readFootprints() {
  ...
  return raw.map((b) => b[1]).filter((ring) => KEEP(ring[0]));
}
...
// Regenerate with: node scripts/fetch-lisbon-terrain-30m.mjs --region ${REGION}
//
...
export const ${EXPORT_NAME}: TerrainGrid = {
  minLat: ${grid.minLat},
  minLng: ${grid.minLng},
